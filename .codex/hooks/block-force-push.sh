#!/usr/bin/env bash
#
# block-force-push.sh — Codex PreToolUse hook (issue #638)
#
...
# stdout and exit 2 (per Codex hook semantics).
#
...
FP_PAYLOAD="$payload" node -e '
  ...
  const reason =
    "Force-push blocked by .codex/hooks/block-force-push.sh (issue #638). " +
    ...
    "required, the human operator must run it manually outside Codex.";
