...
| `src/lib/marvinUrl.ts` *(new)* | `toMarvinUrl`, `resolveImageSrc` | #2, #3 |
| `src/lib/marvinUrl.test.ts` *(new)* | Resolver coverage: 3 syntaxes + edge cases | #4 |
...
| `tests/e2e/preview-images.spec.ts` *(new)* | Playwright: relative, absolute, wikilink, broken | #7 |
