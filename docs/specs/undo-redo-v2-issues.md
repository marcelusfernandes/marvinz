...
5. `handlePickVault` calls `useFileOpsHistory.getState().reset()`.
...
focus** — the upgrade is a proper file-op undo/redo *engine* + redo + a focus
...
### Solução
E2E covering rename→undo→redo, move→undo→redo, trash→undo→redo against the real filesystem. Note (per #151): rename/trash op-*triggers* are native-context-menu-gated in Playwright; drive via the testable paths (drag-move is fully driveable; for rename/trash assert at the engine/IPC seam).
