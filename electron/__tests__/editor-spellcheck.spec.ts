...
function captureContextMenuParams(params: { misspelledWord: string; dictionarySuggestions: string[] }): void {
...
describe('editor:spellcheck-context', () => {
  ...
  it('returns misspelled word and suggestions after context-menu event', () => {
    ...
    expect(spellcheckContextHandler()).toEqual({ misspelledWord: 'helo', suggestions: ['hello', 'helo'] })
  })
  ...
  it('updates state on subsequent context-menu events', () => {
    ...
    expect(spellcheckContextHandler()).toEqual({ misspelledWord: 'teh', suggestions: ['the', 'ten'] })
  })
