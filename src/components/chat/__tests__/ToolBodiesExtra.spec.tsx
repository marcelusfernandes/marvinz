...
describe('WriteCard — structure', () => {
  it('renders chat-tool-card-write wrapper', () => {
    const { container } = render(
      <WriteCard toolUseId="tu1" tool="Write" input={{ file_path: '/vault/note.md', content: 'hello' }} status="running" />,
    )
  ...
  it('sets data-tool attribute', () => {
    const { container } = render(
      <WriteCard toolUseId="tu1" tool="Write" input={{ file_path: '/vault/note.md', content: '' }} status="running" />,
    )
  ...
  it('renders filename pill showing basename', () => {
    const { container } = render(
      <WriteCard toolUseId="tu1" tool="Write" input={{ file_path: '/vault/src/utils.ts', content: '' }} status="running" />,
    )
  ...
  it('pill has data-risk="destructive"', () => {
    const { container } = render(
      <WriteCard toolUseId="tu1" tool="Write" input={{ file_path: '/vault/a.md', content: '' }} status="running" />,
    )
...
describe('WriteCard — subline', () => {
  it('shows "Creating" subline for Write tool while running', () => {
    const { container } = render(
      <WriteCard toolUseId="tu1" tool="Write" input={{ file_path: '/vault/a.md', content: 'line' }} status="running" />,
    )
  ...
  it('shows "Created" subline for Write tool on success', () => {
    const { container } = render(
      <WriteCard toolUseId="tu1" tool="Write" input={{ file_path: '/vault/a.md', content: 'line' }} status="ok" />,
    )
  ...
  it('shows "Denied" subline on denied status', () => {
    const { container } = render(
      <WriteCard toolUseId="tu1" tool="Write" input={{ file_path: '/vault/a.md' }} status="denied" />,
    )
...
describe('AgentCard — structure', () => {
  it('renders chat-tool-card-agent wrapper', () => {
    const { container } = render(
      <AgentCard toolUseId="tu1" tool="Agent" input={{ description: 'Run tests' }} status="running" />,
    )
  ...
  it('renders description text when present', () => {
    render(
      <AgentCard toolUseId="tu1" tool="Agent" input={{ description: 'Run the test suite' }} status="running" />,
    )
  ...
  it('renders subagent_type as a pill', () => {
    const { container } = render(
      <AgentCard toolUseId="tu1" tool="Agent" input={{ subagent_type: 'researcher' }} status="running" />,
    )
  ...
  it('renders prompt when present', () => {
    render(
      <AgentCard toolUseId="tu1" tool="Agent" input={{ prompt: 'Summarise this document' }} status="running" />,
    )
  ...
  it('shows string result in output area', () => {
    ...
    expect(container.querySelector('.chat-tool-agent-output')?.textContent).toContain('Done successfully')
  })

  it('shows errorMessage in output area on error', () => {
    const { container } = render(
      <AgentCard toolUseId="tu1" tool="Agent" input={{}} status="error" errorMessage="Sub-agent crashed" />,
    )
...
describe('GenericToolCard — structure', () => {
  it('renders chat-tool-card-generic wrapper', () => {
    const { container } = render(
      <GenericToolCard toolUseId="tu1" tool="mcp__my_tool" input={{ foo: 'bar' }} status="running" />,
    )
  ...
  it('renders output block on error with data-error=true', () => {
    const { container } = render(
      <GenericToolCard toolUseId="tu1" tool="mcp__tool" input={{}} status="error" errorMessage="oops" />,
    )
...
describe('ToolBody resolver', () => {
  ...
  it('renders WriteCard for tool="Write"', () => {
    const { container } = render(
      <ToolBody toolUseId="tu1" tool="Write" input={{ file_path: '/vault/a.md', content: '' }} status="running" />,
    )
    expect(container.querySelector('.chat-tool-card-write')).toBeInTheDocument()
  })
  ...
  it('does not crash for empty string tool name', () => {
    const { container } = render(
      <ToolBody toolUseId="tu1" tool="" input={{}} status="running" />,
    )
    expect(container.querySelector('.chat-tool-card-generic')).toBeInTheDocument()
  })
})
